...
class MNISTDataset(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        data = np.resize(data,AD_Constants.img_shape_tuple)/255
...
parser.add_argument("--examples", type=int, default=340)
...
dataset = create_torchvision_dataset_wrapper2(MNISTDataset)

#
dataset=dataset(RepeatEncoder(time=time, dt=dt),None,all_files,transformi,
)
...
for (i, dataPoint) in pbar:
    if i > n_iters:
        break
    datum = dataPoint["encoded_image"].view(int(time/dt), 1, 1, 464, 464).to(device)
    #datum = dataPoint["encoded_image"].view(1, 1, 464, 464).to(device)
    ...
    if plot:
        ...
        inpt_axes, inpt_ims = plot_input(
            dataPoint["image"].view(464, 464),
            datum.view(int(time/dt), 215296).sum(0).view(464, 464),
            label=label,
            axes=inpt_axes,
            ims=inpt_ims,
        )
        ...
        weights_im = plot_weights(
            get_square_weights(C1.w, 23, 464), im=weights_im, wmin=-2, wmax=2
        )
...
model = NN(n_neurons, 2).to(device)
...
for epoch, _ in pbar:
    ...
    for i, (s, l) in enumerate(training_pairs):
        ...
        optimizer.zero_grad()
        outputs = model(s)
        label = torch.zeros(1, 1, 2).float().to(device)
        label[0, 0, l] = 1.0
...
for (i, dataPoint) in pbar:
    if i > n_iters:
        break
    datum = dataPoint["encoded_image"].view(time, 1, 1, 464, 464).to(device)
    label = dataPoint["label"]
    ...
    if plot:
        inpt_axes, inpt_ims = plot_input(
            dataPoint["image"].view(464, 464),
            datum.view(time, 215296).sum(0).view(464, 464),
            label=label,
            axes=inpt_axes,
            ims=inpt_ims,
        )
